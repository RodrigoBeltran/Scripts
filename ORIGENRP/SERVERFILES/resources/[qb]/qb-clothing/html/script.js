...
$(".clothing-menu-option-item-left, .clothing-menu-option-item-right, .clothing-menu-header-btn, .clothing-menu-header-camera-btn, .clothing-menu-option-button, .change-camera-button, .clothing-menu-myOutfit-option-button, .clothing-menu-myOutfit-option-button-remove").mouseenter(function(){
    over_button.play();
});

$(".clothing-menu-option-button, .clothing-menu-myOutfit-option-button-remove").click(function(){
    click.play();
});
...
QBClothing.ReloadOutfits = function(outfits) {
    ...
    $.each(outfits, function(index, outfit){
        var elem = '<div class="clothing-menu-option" data-myOutfit="'+(index + 1)+'"> <div class="clothing-menu-option-header"><p>'+outfit.outfitname+'</p></div><div class="clothing-menu-myOutfit-option-button"><p>Vestir</p></div><div class="clothing-menu-myOutfit-option-button-remove"><p>Eliminar</p></div></div>'
...
QBClothing.Open = function(data) {
    ...
    $.each(data.menus, function(i, menu){
        ...
        if (menu.menu == "myOutfits") {
            $.each(menu.outfits, function(index, outfit){
                var elem = '<div class="clothing-menu-option" data-myOutfit="'+(index + 1)+'"> <div class="clothing-menu-option-header"><p>'+outfit.outfitname+'</p></div><div class="clothing-menu-myOutfit-option-button"><p>SELECCIONAR OUTFIT</p></div><div class="clothing-menu-myOutfit-option-button-remove"><p>ELIMINAR</p></div></div>'
                $(".clothing-menu-myOutfits-container").append(elem)
        ...
        if(menu.label=="Peluquería"){
            $("#save-outfit").hide();
            $("#z_ojos, #pecas, #nariz, #altura-nariz, #long-nariz, #alt-hueso-nariz, #pico-nariz, #g-pico-nariz, #ancho-mandibula, #long-mandibula, #ancho-mejillas, #apertura-ojos, #anchura-labios, #anchura-mandibula, #long-mandibula, #altura-barbilla, #long-barbilla, #ancho-barbilla, #menton, #anchura-cuello, #arrugas").hide();
        } else {
            $("#save-outfit").show();
            $("#z_ojos, #pecas, #nariz, #altura-nariz, #long-nariz, #alt-hueso-nariz, #pico-nariz, #g-pico-nariz, #ancho-mandibula, #long-mandibula, #ancho-mejillas, #apertura-ojos, #anchura-labios, #anchura-mandibula, #long-mandibula, #altura-barbilla, #long-barbilla, #ancho-barbilla, #menton, #anchura-cuello, #arrugas").show();
...
QBClothing.SetMaxValues = function(maxValues) {
    $.each(maxValues, function(i, cat){
        if (cat.type == "character") {
            ...
            $(itemMax).html("<p>Elementos: " + maxValues[containers.data('type')].item + "</p>")
