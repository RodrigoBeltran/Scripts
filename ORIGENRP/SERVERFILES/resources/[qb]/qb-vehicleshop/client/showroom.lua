            ...
            if dist < 2.5 then
                if not QB.ShowroomVehicles[ClosestVehicle].inUse then
                        ...
                        if not vehshop.opened then
                            if not buySure then
                                if PlayerJob.name == 'cardealer' then
                                    DrawText3Ds(QB.ShowroomVehicles[ClosestVehicle].coords.x, QB.ShowroomVehicles[ClosestVehicle].coords.y, QB.ShowroomVehicles[ClosestVehicle].coords.z + 1.70, '~g~G~w~ - Cambiar Vehiculo (~g~'..displayName..'~w~), ~g~E~w~ - Vender Vehiculo (~g~$'..vehPrice..'~w~)')
                                else
                                    DrawText3Ds(QB.ShowroomVehicles[ClosestVehicle].coords.x, QB.ShowroomVehicles[ClosestVehicle].coords.y, QB.ShowroomVehicles[ClosestVehicle].coords.z + 1.70, '~g~E~w~ - Comprar Vehiculo ($'..vehPrice..'~w~)')
                                end
                            elseif buySure then
                                DrawText3Ds(QB.ShowroomVehicles[ClosestVehicle].coords.x, QB.ShowroomVehicles[ClosestVehicle].coords.y, QB.ShowroomVehicles[ClosestVehicle].coords.z + 1.65, 'Estas seguro? | ~g~[7]~w~ Confirmar -/- ~r~[8]~w~ Cancelar')
                            end
                        elseif vehshop.opened then
                            if modelLoaded then
                                DrawText3Ds(QB.ShowroomVehicles[ClosestVehicle].coords.x, QB.ShowroomVehicles[ClosestVehicle].coords.y, QB.ShowroomVehicles[ClosestVehicle].coords.z + 1.65, 'Eligiendo un Vehiculo')
                            else
                                DrawText3Ds(QB.ShowroomVehicles[ClosestVehicle].coords.x, QB.ShowroomVehicles[ClosestVehicle].coords.y, QB.ShowroomVehicles[ClosestVehicle].coords.z + 1.65, 'Cargando Modelo')
                            end
                        end
                    else
                        DrawText3Ds(QB.ShowroomVehicles[ClosestVehicle].coords.x, QB.ShowroomVehicles[ClosestVehicle].coords.y, QB.ShowroomVehicles[ClosestVehicle].coords.z + 1.65, 'En uso')
                    end
                    ...
                    if IsControlJustPressed(0, 38) then
                        if not vehshop.opened then
                            QBCore.Functions.TriggerCallback('qb-vehicleshop:checkIsVehicleShopIsOpen', function(can)
                                if PlayerJob.name ~= 'cardealer' then
                                    if can == false then
                                        if not buySure then
                                            buySure = true
                                        ...
                                        QBCore.Functions.Notify('No puedes comprar un vehículo ya que el concesionario esta abierto')
                                    end
                                else
                                    if can then
                                        local class = QBCore.Shared.Vehicles[QB.ShowroomVehicles[ClosestVehicle].chosenVehicle]["category"]
                                        local vehicle = QB.ShowroomVehicles[ClosestVehicle].chosenVehicle
                                        local player, distance = QBCore.Functions.GetClosestPlayer()
                                        if player ~= -1 and distance <= 3.0 then
                                            TriggerServerEvent('qb-vehicleshop:server:sellVehicle', vehicle, class, GetPlayerServerId(player))
                                        else
                                            QBCore.Functions.Notify('No hay jugadores cerca')
                                        end
                                    else
                                        QBCore.Functions.Notify('Necesitas tener el local abierto para poder vender este vehiculo')
                                    end
                                end
                            end)
                        end
                    ...
                    if IsDisabledControlJustPressed(0, 162) then
                        QBCore.Functions.Notify('Compra cancelada', 'error')
                        buySure = false
                    end
                    DisableControlAction(0, 161, true)
                    DisableControlAction(0, 162, true)
                elseif QB.ShowroomVehicles[ClosestVehicle].inUse then
                    DrawText3Ds(QB.ShowroomVehicles[ClosestVehicle].coords.x, QB.ShowroomVehicles[ClosestVehicle].coords.y, QB.ShowroomVehicles[ClosestVehicle].coords.z + 0.5, 'En Uso')
                end
...
RegisterNetEvent('qb-vehicleshop:client:offerVehicle')
AddEventHandler('qb-vehicleshop:client:offerVehicle', function(vehiclePrice, vehicleLabel, vehicle, _source)
    while true do
        Wait(0)
        QBCore.Functions.HelpNotify('Presiona ~INPUT_CONTEXT~ para comprar un '..vehicleLabel..' por ' ..vehiclePrice.. '$ o Presiona BackSpace para rechazar la oferta')
        if IsControlJustPressed(1, 38) then
            TriggerServerEvent('qb-vehicleshop:server:buyVehicleWithCardealer', vehiclePrice, vehicleLabel, vehicle, _source)
            break
        elseif IsControlJustPressed(1, 178) then
            QBCore.Functions.Notify('Has rechazado la oferta')
            break
        end
    end
end)
