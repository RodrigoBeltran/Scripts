(function() {
    let MenuTpl =
        '<div id="menu_{{_namespace}}_{{_name}}" class="dialog fadeIn {{#isBig}}big{{/isBig}}">' +
        '<div class="shake"><div class="head">{{title}}</div></div>' +
    ...
    window.ESX_MENU = {};
    ESX_MENU.ResourceName = 'qb-menu-dialog';
    ESX_MENU.opened = {};
    ESX_MENU.focus = [];
    ESX_MENU.pos = {};
    ...
    ESX_MENU.render = function() {
        ...
        $(".dialog").removeClass("salida").addClass("fadeIn");
        $(menuContainer).show();
        $("#inputText").focus();
    };
    ...
    window.onData = (data) => {
        switch (data.action) {
            ...
            case 'closeMenu':
                {
                    $(".dialog").removeClass('fadeIn').addClass("salida");
                    setTimeout(function() {
