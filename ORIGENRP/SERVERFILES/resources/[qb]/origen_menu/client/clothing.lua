...
local Drawables = {
	...
	["Shoes"] = {
		Drawable = 6,
		Table = {Standalone = true, Male = 85, Female = 84},
...
local Extras = {
	["Shirt"] = {
		Drawable = 11,
		Table = {
			Standalone = true, Male = 352, Female = 15,
			Extra = { 
	...
	["Pants"] = {
		Drawable = 4,
		Table = {Standalone = true, Male = 92, Female = 15},
...
function ToggleClothing(whic, extra)
	...
	if which ~= "Mask" then
		if not Gender then Notify("Esta ped no admite esto.") return false end -- We cancel the command here if the person is not using a multiplayer model.
	end
	...
	if not Toggle.Table.Standalone then -- "Standalone" is for things that dont require a variant, like the shoes just need to be switched to a specific drawable. Looking back at this i should have planned ahead, but it all works so, meh!
		...
		Notify("No hay ninguna variante para esto.") return
	else
		if not LastEquipped[which] then
	...
	Notify("Ya estás usando esto") return false
end
...
function ToggleProps(whic)
	...
	if not Prop.Variants then
		...
		Notify("No tienes nada que quitarte.") return false
	else
		local Gender = IsMpPed(Ped)
		if not Gender then Notify("Este modelo no soporta esta opción.") return false end -- We dont really allow for variants on ped models, Its possible, but im pretty sure 95% of ped models dont really have variants.
		...
		Notify("No parece que haya ninguna variante de esto") return false
	end
end
