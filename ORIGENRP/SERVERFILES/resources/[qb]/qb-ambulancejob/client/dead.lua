	...
	while true do
		...
		if NetworkIsPlayerActive(player) then
            ...
            elseif IsEntityDead(playerPed) and InLaststand and not isDead then
                SetLaststand(false)
                TriggerServerEvent("guille_gangsv2:server:addDead")
		...
		if isDead or InLaststand then
            ...
            if isDead then
                if not isInHospitalBed then 
                    ...
                    TriggerEvent('hud:client:mostrar-respawn', 'Reaparición disponible en <span class="tiempo">'..math.ceil(deathTime)..'</span> segundos')
                    else
                    TriggerEvent('hud:client:mostrar-respawn', 'Mantén pulsada la tecla <span class="tiempo">E ('..hold..')</span> para ser atendido por '..Config.BillCost..'$')
            ...
            elseif InLaststand then
                ...
                if LaststandTime > Laststand.MinimumRevive then
                 --   DrawTxt(0.94, 1.44, 1.0, 1.0, 0.6, "Te desangraras" .. math.ceil(LaststandTime) .. "segundos", 255, 255, 255, 255)
                    TriggerEvent('hud:client:mostrar-muerte', math.ceil(LaststandTime))
                else
                  --  DrawTxt(0.845, 1.44, 1.0, 1.0, 0.6, "Te desangraras" .. math.ceil(LaststandTime) .. "segundos, pide ayuda!", 255, 255, 255, 255)
                    TriggerEvent('hud:client:mostrar-muerte', math.ceil(LaststandTime))
                end
