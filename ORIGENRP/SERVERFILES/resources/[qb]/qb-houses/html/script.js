...
$('document').ready(function() {
    ...
    window.addEventListener('message', function(event) {
        ...
        if (item.type == "buyOption") {
            ...
            $(".decorate-confirm").find("p").html("¿Estás seguro de que deseas comprar el artículo por $"+selectedObjectData.price+"?");
        }
            ...
            if (selectedObjectData != null && $(".decorate-confirm").css('display') != 'block') {
                ...
                $.post('https://qb-houses/setupMyObjects', JSON.stringify({}), function(myObjects){
                    ...
                    $.each(myObjects, function(i, object){
                        var elem = '<div class="decorate-item" id="myobject-'+i+'" data-type="myObject"><span id="decorate-item-name"><b>Objecto: </b>'+object.hashname+'</span><span id="decorate-item-category"><strong>Precio: </strong><span id="item-price">COMPRADO</span></span></div>';
...
Decorations.SetupCategorys = function() {
    ...
    $(".footer-btn, .btn-spawnobj").mouseenter(function(){
        over_button.play();
    });
    $(".footer-btn, .btn-spawnobj").on("click", function(){
        click.play();
    });
}
...
$(document).on('click', '.footer-btn', function(){
    ...
    if (selectedCategory != "remove-owned-obj") {
        ...
        $.each(houseCategorys[selectedCategory].items, function(i, item){
            var elem = '<div class="decorate-item" id="object-'+i+'" data-type="newObject"><span id="decorate-item-name"><b>Objecto: </b>'+(item.label).charAt(0).toUpperCase() +''+(item.label).substr(1).toLowerCase()+'</span><span id="decorate-item-category"><strong>Precio: </strong><span id="item-price">$'+item.price+'</span></span></div>';
            $('.decorate-items').append(elem);
            $('#object-'+i).data('objectData', item);
        });
        $(".decorate-item").mouseenter(function(){
            over_button.play();
        });
