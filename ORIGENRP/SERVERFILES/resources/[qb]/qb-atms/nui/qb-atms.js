...
function populateCurrentCards(data) {
    ...
    $.each(data.cards, function(index, card) {
        ...
        if(card.cardActive !== undefined && card.cardActive !== null && card.cardActive === true) {
            cardStatus = '<span class="text-dark">(Activa)</span> ';
            $('#cardsDisplay').append('<div class="card mb-3 mx-auto" style="width:100%; background:rgb(255 255 255); color: #000000"><div class="row no-gutters"><div class="col-2 my-auto text-center"><img src="images/' + image + '" class="img-fluid" width="100" alt=""></div><div class="col-8"><div class="card-body p-0"><h5 class="card-title pb-0 pt-1 pl-2 mb-0" style="color: #000000" id="cardOwner-' + index + '">' + card.name + '</h5><p class="card-text pl-2"><small class="text-muted"><span style="color: #000000" id="cardType-' + index + '">' + cardStatus + '' + card.cardType + '</span><br><span style="color: #000000; font-size: 12px;" id="cardNumber-' + index + '">' + cardNumber + '</span></small></p></div></div><div class="col-2 my-auto pr-1"><button class="btn btn-dark btn-sm btn-block" style="font-size: 12px;" id="cardBtn-' + index + '" data-action="useCard">Acceder</button></div></div></div>');
        } else {
            cardStatus = '';
            $('#cardsDisplay').append('<div class="card mb-3 mx-auto" style="width:100%;background:rgb(255 255 255); color: #000000"><div class="row no-gutters"><div class="col-2 my-auto text-center"><img src="images/' + image + '" class="img-fluid" width="100" alt=""></div><div class="col-8"><div class="card-body p-0"><h5 class="card-title pb-0 pt-1 pl-2 mb-0" style="color: #000000" id="cardOwner-' + index + '">' + card.name + '</h5><p class="card-text pl-2"><small class="text-muted"><span style="color: #000000" id="cardType-' + index + '">' + cardStatus + '' + card.cardType + '</span><br><span style="color: #000000; font-size: 12px;" id="cardNumber-' + index + '">' + cardNumber + '</span></small></p></div></div><div class="col-2 my-auto pr-1"><button class="btn btn-block" style="background-color: #a7000b; color: white; font-size: 12px;" id="cardBtn-' + index + '" data-action="removeCard">Salir</button></div></div></div>');
        }
