...
function mechStart(inicoords, carcoords, vehicle, carh)
    ...
    while true do
        ...
        if IsPedInVehicle(ped, elecveh) then
            ...
            QBCore.Functions.Notify('Ve a por la mercancia')
            break
        end
        Citizen.Wait(wait)
    end
...
function createMarkersMech(ped, inicoords, mechveh)
    ...
    if mechdone ~= Config.Mech.MaxRepairs then
        ...
        while point do
            ...
            dist = #(pcoords - vector3(Config.Mech.locations[mech].x, Config.Mech.locations[mech].y, Config.Mech.locations[mech].z))
            ...
            if dist < 40 then
                ...
                if dist < 5 then
                    ...
                    if DoesEntityExist(mechveh) then
                        QBCore.Functions.HelpNotify("Presiona ~r~E~w~ para recoger la carga", merccoords + vector3(0.0, 0.0, 1.5))
...
function createMarkersMechMerc(mechveh)
    ...
    while point do
        ...
        dist = #(pcoords - vector3(1218.64, -3230.92, 4.69))
        ...
        if dist < 5 then
            ...
            if dist < 3 then
                wait = 5
                if DoesEntityExist(mechveh) then
                    QBCore.Functions.HelpNotify("Presiona ~r~E~w~ para entregar la carga", vector3(1218.64, -3230.92, 4.69) + vector3(0.0, 0.0, 1.5))
                    if IsControlJustPressed(1, 38) then
                        DeleteObject(mechobject)
                        mechdone = mechdone + 1
                        TriggerServerEvent('esx_actividades:pay', "puerto")
                        RemoveBlip(mercblip)
...
function finishJob(mechveh)
    QBCore.Functions.Notify('Has terminado el trabajo, dirigete de vuelta a la sede')
    CreateMechBlip(Config.Mech.FinishPoint)
    while true do
        local wait = 1500
        local pcoords = GetEntityCoords(PlayerPedId())
        dist = #(pcoords - Config.Mech.FinishPoint)
        if dist < 2 then
            wait = 0
            QBCore.Functions.HelpNotify("Presiona ~g~E~w~ para dejar el vehículo", Config.Mech.FinishPoint)
            if IsControlJustPressed(1, 38) then
...
function CreateMechBlipMerc()
	...
	SetBlipRouteColour(mercblip, 4)
	BeginTextCommandSetBlipName("STRING")
	AddTextComponentString("Entrega de mercancia")
	EndTextCommandSetBlipName(mercblip)

    QBCore.Functions.Notify('Ahora ve a entregar la mercancia')
end
