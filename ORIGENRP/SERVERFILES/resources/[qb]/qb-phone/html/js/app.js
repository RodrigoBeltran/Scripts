...
QB.Phone.Functions.OpenApp = function(PressedApplication, AppObject) {
    if (AppObject.length !== 0) {
        ...
        QB.Phone.Notifications.Add("fas fa-exclamation-circle", "Sistema", QB.Phone.Data.Applications[PressedApplication].tooltipText + " estará disponible pronto.")
    }
}
...
$(document).on('click', '.phone-application', function(e) {
    ...
    let PressedApplication = $(this).data('app');
    let AppObject = $("." + PressedApplication + "-app");

    QB.Phone.Functions.OpenApp(PressedApplication, AppObject)
});
...
QB.Phone.Functions.CloseApp = function() {
    if (QB.Phone.Data.currentApplication === null) {
...
QB.Phone.Functions.Close = function() {
    ...
    if (usingVoiceDictation) {
        ...
        $('#voice-tweet-button').html("Presiona voz")
    }
    QB.Phone.Animations.BottomSlideDown('.container', 300, -80);
    $.post('https://qb-phone/Close');
    QB.Phone.Data.IsOpen = false;
}

QB.Phone.Functions.ForceClose = function() {
    ...
    QB.Phone.Animations.BottomSlideDown('.container', 300, -80);
    $.post('https://qb-phone/ForceClose');
    QB.Phone.Data.IsOpen = false;
}
        ...
        switch (event.data.action) {
            ...
            case "UpdateChat":
                if (QB.Phone.Data.currentApplication == "whatsapp") {
                    ...
                    if (OpenedChatData.isGroup == event.data.chatData.isGroup && (OpenedChatData.id == event.data.chatData.id || OpenedChatData.id == -1)) {
                        console.log('Chat reloaded', event.data.chatData.id)
            ...
            case "UpdateCallTime":
                ...
                if (!QB.Phone.Data.IsOpen) {
                    ...
                    $(".call-notifications-title").html("En llamada: (" + timeString + ")");
                    $(".call-notifications-content").html("Hablando con " + event.data.Name);
                ...
                $(".phone-currentcall-title").html("En llamada (" + timeString + ")");
                break;
