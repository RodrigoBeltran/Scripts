...
$(document).on('click', '#sell-crypto', function(e){
    ...
    if ((Coins !== "") && (Price !== "")) {
        if (CryptoData.Portfolio >= parseInt(Coins)) {
            $.post('https://qb-phone/SellCrypto', JSON.stringify({
                ...
                if (CryptoData !== false) {
                    ...
                    CloseCryptoPage()
                    QB.Phone.Data.PlayerData.money.bank = parseInt(QB.Phone.Data.PlayerData.money.bank) + parseInt(Price);
                    QB.Phone.Notifications.Add("fas fa-university", "QBank", "&#36; "+Price+",- ha sido agregado a tu saldo!", "#badc58", 2500);
                } else {
        ...
        QB.Phone.Notifications.Add("fas fa-chart-pie", "Crypto", "¡Complete todos los campos!", "#badc58", 1500);
    }
    CloseCryptoPage();
    e.handled = false;
}
            ...
            if (WalletId !== CryptoData.WalletId) {
                $.post('https://qb-phone/TransferCrypto', JSON.stringify({
                    ...
                    if (CryptoData == "notenough") {
                        ...
                        QB.Phone.Notifications.Add("fas fa-university", "Crypto", "este Wallet-ID no existe!", "#badc58", 2500);
                    } else {
                        UpdateCryptoData(CryptoData)
                        CloseCryptoPage()
                ...
                QB.Phone.Notifications.Add("fas fa-university", "Crypto", "No puedes transferirte a ti mismo ..", "#badc58", 2500);
            }
        } else {
            QB.Phone.Notifications.Add("fas fa-chart-pie", "Crypto", "No tienes suficientes Qbits ...", "#badc58", 1500);
        }
