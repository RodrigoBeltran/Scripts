...
function AddEmoteMenu(menu)
    ...
    local Sex = _menuPool:AddSubMenu(submenu, Config.Languages[lang]['Sex'], "", "", Menuthing, Menuthing)
    ...
    table.insert(EmoteTable, Config.Languages[lang]['danceemotes'])
    table.insert(EmoteTable, Config.Languages[lang]['Sex'])
    ...
    if not Config.SqlKeybinding then
      ...
      keyinfo =  NativeUI.CreateItem(Config.Languages[lang]['keybinds'], Config.Languages[lang]['keybindsinfo'].." /bind keyboard NOMBREDELATECLA \"COMANDO\" en el F8 para bindearte una animación.")
    ...
    for a,b in pairsByKeys(DP.Sex) do
      x,y,z = table.unpack(b)
      sexitem = NativeUI.CreateItem(z, "/e ("..a..")")
      Sex:AddItem(sexitem)
      table.insert(SexTable, a)
    end
    ...
    if Config.SharedEmotesEnabled then
      for a,b in pairsByKeys(DP.Shared) do
        ...
        if otheremotename == nil then
          ...
          shareitem = NativeUI.CreateItem(z, "/nearby ("..a..") "..Config.Languages[lang]['makenearby'].." (~y~"..otheremotename..")")
        end
    ...
    Sex.OnItemSelect = function(sender, item, index)
      EmoteMenuStart(SexTable[index], "Sex")
    end
